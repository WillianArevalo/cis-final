<div>
    @push('title', 'Mi proyecto social')
    <div class="flex flex-col gap-6">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            <div class="space-y-1">
                <flux:heading size="xl">
                    {{ $project ? 'Mi proyecto social' : 'Crear proyecto social' }}
                </flux:heading>
                <flux:text class="text-sm">
                    {{ $project ? 'Actualiza la informacion del proyecto social.' : 'Completa la informacion para crear tu proyecto social.' }}
                </flux:text>
            </div>
            <flux:button variant="ghost" icon="arrow-left" as="link" href="{{ route('scholar.home') }}" wire:navigate>
                Volver
            </flux:button>
        </div>
        ...
        @if (!$project)
            <flux:callout color="blue" icon="information-circle">
                ...
                <flux:callout.text>
                    No tienes un proyecto social asignado. Completa el formulario para crear tu proyecto y enviarlo al
                    cómite para su posterior aceptación. Asegurate que de todos los integrantes del proyecto solo uno de
                    ellos cree el proyecto para evitar duplicados. Una vez creado el proyecto, podrás editar su
                    información.
                </flux:callout.text>
            </flux:callout>
        @endif
        ...
        <form wire:submit="save" class="flex flex-col gap-6">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                ...
                @if ($project)
                    <flux:input label="Comunidad"
                        value="{{ $project->community ? $project->community->name : 'Sin definir' }}" disabled />
                @else
            ...
            @if (!$project)
                <flux:pillbox label="Integrantes del proyecto" class="w-full" wire:model.defer="scholarshipsIds"
                    searchable search:placeholder="Buscar becarios..." multiple
                    placeholder="Selecciona los integrantes del proyecto"
                    description:trailing="Asegurate de seleccionar solos los integrantes de tu proyecto.">
                    @foreach ($scholars as $scholar)
                        <flux:pillbox.option value="{{ $scholar->id }}" label="{{ $scholar->name }}" />
                    @endforeach
                </flux:pillbox>
            @endif
            ...
            <div class="grid gap-6 md:grid-cols-3">
                ...
                <div class="flex flex-col gap-4">
                    <flux:file-upload wire:model="document" label="Documento del proyecto" accept="application/pdf"
                        max-size="5120">
                        <flux:file-upload.dropzone heading="Sube el documento en PDF" with-progress
                            text="PDF hasta 5MB" inline />
                    </flux:file-upload>
                ...
                <div class="flex flex-col gap-4">
                    <flux:file-upload wire:model="map" label="Mapa del proyecto" accept="image/*" max-size="5120">
                        <flux:file-upload.dropzone heading="Sube una imagen del mapa" with-progress
                            text="PNG, JPG, JPEG, WEBP hasta 5MB" inline />
                    </flux:file-upload>

                    @if ($currentMap && Storage::disk('public')->exists($currentMap) && !$map)
                        <flux:file-item heading="{{ basename($currentMap) }}"
                            image="{{ Storage::url($currentMap) }}"
                    ...
                    @if ($map)
                        <flux:file-item heading="{{ $map->getClientOriginalName() }}"
                            image="{{ $map->temporaryUrl() }}" size="{{ $map->getSize() }}">
                            <x-slot name="actions">
            ...
            <div class="flex flex-col items-center justify-center gap-2 sm:flex-row">
                <flux:button variant="ghost" type="button" as="link" icon="x-mark"
                    href="{{ route('scholar.home') }}" wire:navigate class="w-full sm:w-max">
                    Cancelar
                </flux:button>
