    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <flux:button variant="ghost" icon="arrow-left" as="link" href="{{ route('scholar.reports.index') }}"
                wire:navigate class="w-full sm:w-max">
                Volver
            </flux:button>
            @if ($report?->is_editable)
                <flux:button variant="primary" icon="pencil-square" as="link"
                    href="{{ route('scholar.reports.edit', $report->id) }}" wire:navigate>
                    Editar reporte
                </flux:button>
            @endif
        </div>
                    ...
                    @if ($image->path && Storage::disk('public')->exists($image->path))
                        <button type="button"
                            ...
                            <img src="{{ Storage::url($image->path) }}" alt="Evidencia del reporte"
                                class="size-20 md:size-40 object-cover transition duration-300 group-hover:scale-105">
                        </button>
                    @endif
                @endforeach
            </div>
